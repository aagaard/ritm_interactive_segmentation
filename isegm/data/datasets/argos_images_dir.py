...
class ArgosImagesDirDataset(ISDataset):
    def __init__(self, dataset_path, images_dir_name="images", masks_dir_name="masks", **kwargs):
        super(ArgosImagesDirDataset, self).__init__(**kwargs)
        ...
        samples = {x.stem.replace("_color", ""): {"image": x, "masks": []} for x in images_list}
        for mask_path in self._insts_path.glob("*.*"):
            mask_name = mask_path.stem.replace("_instance", "")
            ...
            if mask_name_split[-1].isdigit():
                ...
                assert mask_name in samples, f"{mask_name=}, {mask_name_split=}"
                samples[mask_name]["masks"].append(mask_path)

        samples = {k: v for k, v in samples.items() if len(v["masks"]) > 0}
